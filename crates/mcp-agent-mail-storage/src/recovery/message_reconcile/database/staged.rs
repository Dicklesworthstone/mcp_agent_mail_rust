...
use super::{
    PreparedMessage, merge_surviving_metadata, restore_inbox_metadata,
    validate_surviving_message,
};
...
impl StagedMessages {
    fn open(repo: &Repository) -> Result<Option<Self>, String> {
        ...
        snapshot.write_all(&bytes).map_err(|error| error.to_string())?;
        drop(bytes);
    ...
    fn read(
        ...
        if (1..=3).any(|stage| self.index.get_path(key, stage).is_some()) {
            return Err("staged archive message has unresolved merge stages; preserved".to_string());
        }
        ...
        if kind != ObjectType::Blob
            ...
            || size > *remaining
        {
            return Err("staged archive message byte budget exceeded or object is not a blob".to_string());
        }
        *remaining -= size;
        ...
        let (frontmatter, body) = text
            ...
            .ok_or_else(|| "staged archive message has invalid canonical frontmatter".to_string())?;
    ...
    fn reject_other_canonical_id(
        ...
        for entry in self.index.iter() {
            ...
            if year.len() != 4
                || month.len() != 2
                || !year.bytes().chain(month.bytes()).all(|byte| byte.is_ascii_digit())
                || !name.ends_with(".md")
            {
                continue;
            }
...
mod tests {
    ...
    fn stage_and_retain(
        ...
        let bytes = crate::render_message_bundle_content(message, body).unwrap().into_bytes();
    ...
    fn staged_only_reply_restores_exact_metadata_and_bcc_without_resending() {
        for source in ["canonical", "outbox", "inbox"] {
            ...
            assert!(super::super::CommittedMessages::open(&archive)
                .unwrap().read(&archive, path).unwrap().is_none());
    ...
    fn staged_disagreement_preserves_head_index_and_all_retained_evidence() {
        ...
        let first_bytes = stage_and_retain(&repo, &archive, &paths.canonical, &full, &original.body, &first);
        ...
        let second_bytes = stage_and_retain(&repo, &archive, &paths.inbox[0], &inbox, &original.body, &second);
        ...
        assert!(error.contains("staged archive metadata disagree"), "{error}");
    ...
    fn staged_only_duplicate_canonical_id_refuses_before_publication() {
        ...
        stage_and_retain(
            &repo, &archive, &paths.inbox[0], &inbox, &original.body,
            &config.storage_root.join("retained-inbox.md"),
        );
        let other = archive.root.join("messages/2025/01/other__9.md");
        stage_and_retain(
            &repo, &archive, &other, &full, &original.body,
            &config.storage_root.join("retained-other.md"),
        );
        let index_before = std::fs::read(repo.path().join("index")).unwrap();
        let head_before = repo.head().ok().and_then(|head| head.target());
        let error = reconcile_prepared(&config, &original).unwrap_err();
        assert!(error.contains("canonical message id 9 already staged"), "{error}");
        assert!(!paths.canonical.exists());
        assert!(!paths.outbox.exists());
        assert!(paths.inbox.iter().all(|path| !path.exists()));
        assert_eq!(std::fs::read(repo.path().join("index")).unwrap(), index_before);
    ...
    fn staged_inbox_cannot_authorize_changed_body_identity_routing_or_bcc() {
        for field in ["body", "id", "project_slug", "to", "bcc"] {
            ...
            let body = if field == "body" { "different" } else { &original.body };
            let evidence = config.storage_root.join("retained-invalid.md");
            let bytes = stage_and_retain(&repo, &archive, &paths.inbox[0], &message, body, &evidence);
            let index_before = std::fs::read(repo.path().join("index")).unwrap();
            let head_before = repo.head().ok().and_then(|head| head.target());
    ...
    fn staged_snapshot_does_not_mix_index_generations() {
        ...
        stage_and_retain(&repo, &archive, &paths.canonical, &first, &original.body,
            &config.storage_root.join("retained-first.md"));
        let snapshot = StagedMessages::open(&repo).unwrap().unwrap();
        let mut second = first.clone();
        second["reply_to"] = json!(8);
        stage_and_retain(&repo, &archive, &paths.canonical, &second, &original.body,
            &config.storage_root.join("retained-second.md"));
        let mut remaining = super::super::super::MAX_BUNDLE_BYTES;
        let (observed, _) = snapshot.read(&repo, &archive, &paths.canonical, &mut remaining).unwrap().unwrap();
        assert_eq!(observed, first);
        let fresh = StagedMessages::open(&repo).unwrap().unwrap();
        let (observed, _) = fresh.read(&repo, &archive, &paths.canonical, &mut remaining).unwrap().unwrap();
    ...
    fn staged_conflicts_nonregular_entries_and_missing_objects_are_not_absence() {
        for problem in ["merge", "symlink", "missing"] {
            ...
            message["reply_to"] = json!(7);
            stage_and_retain(&repo, &archive, &paths.canonical, &message, &original.body,
                &config.storage_root.join("retained-staged.md"));
            let relative = crate::rel_path_cached(&archive.canonical_repo_root, &paths.canonical).unwrap();
            let mut index = repo.index().unwrap();
            ...
            match problem {
                ...
                _ => entry.id = Oid::hash_object(ObjectType::Blob, b"not stored in this repository").unwrap(),
            }
    ...
    fn staged_index_and_blob_budgets_refuse_without_mutation() {
        ...
        let repo = Repository::open(&archive.repo_root).unwrap();
        stage_and_retain(&repo, &archive, &paths.canonical, &original.message, &original.body,
            &config.storage_root.join("retained-staged.md"));
        let snapshot = StagedMessages::open(&repo).unwrap().unwrap();
        assert!(snapshot.read(&repo, &archive, &paths.canonical, &mut 1).unwrap_err().contains("budget"));
