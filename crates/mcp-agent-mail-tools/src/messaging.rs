...
mod tests {
    ...
    fn contact_blocked_error_payload_has_no_data_field() {
        ...
        assert_eq!(payload.get("type").and_then(serde_json::Value::as_str), Some("CONTACT_BLOCKED"));
        assert_eq!(
            payload.get("message").and_then(serde_json::Value::as_str),
            Some("Recipient is not accepting messages.")
        );
        assert_eq!(
            payload.get("recoverable").and_then(serde_json::Value::as_bool),
            Some(true)
        );
        assert!(
