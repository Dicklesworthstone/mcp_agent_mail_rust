...
PowerShell enforces the same exact-tag SHA-256/Sigstore identity contract before `Expand-Archive`; it also requires `cosign` on `PATH`. The ZIP must contain exactly flat `am.exe` and `mcp-agent-mail.exe`, and their staged and post-install version lines must exactly match the requested release. Options: `-Version vX.Y.Z`, `-Dest PATH`, `-Force`, `-Verify` (explicitly request the already-default cryptographic checks), and `-NoVerify` (the **unsafe** escape that skips only those checksum and Sigstore checks). Archive-member and exact-version checks still run with `-NoVerify`.
