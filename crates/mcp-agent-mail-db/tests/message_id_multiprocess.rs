...
fn run_parent() {
    ...
    let spawn_worker = |name: &'static str, out_name: &'static str| {
    ...
    // allows.
    let mut worker_a = spawn_worker("A", "A");
    let mut worker_b = spawn_worker("B", "B");
...
fn run_worker(db_path: &str) {
    ...
    runtime.block_on(async {
        ...
        if name == "A" {
            // Worker A owns initialization: the schema is live once the
            // project and agent exist, so raise the gate for the parent.
        ...
        let canonical = std::path::Path::new(&storage_root)
