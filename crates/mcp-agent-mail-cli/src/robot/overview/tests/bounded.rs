...
fn pending_recipient_fanout_is_bounded_and_keeps_multiplicity() {
    ...
    for agent in 1..=recipients {
        ...
        execute(&conn, &format!("INSERT INTO message_recipients VALUES (1, {agent}, NULL, NULL)"));
    }
    execute(&conn, "COMMIT");
    let (rows, work) = build_at(&conn, now).expect("bounded fanout");
    assert_eq!((rows[0].unread, rows[0].urgent, rows[0].ack_overdue),
    ...
    assert_eq!(work.message_lookup_rows, recipients.div_ceil(OVERVIEW_PAGE_ROWS));
...
fn release_lookups_ignore_unrelated_history_and_preserve_null_releases() {
    ...
    let now = ACK_OVERDUE_THRESHOLD_US * 10;
    execute(&conn, "INSERT INTO projects VALUES (1, 'alpha')");
    execute(&conn, "CREATE INDEX idx_expiry ON file_reservations(expires_ts)");
    execute(&conn, "BEGIN");
    for id in 1..=OVERVIEW_PAGE_ROWS * 4 {
        execute(&conn, &format!("INSERT INTO file_reservations VALUES ({id}, 777, 0, 1)"));
        execute(&conn, &format!("INSERT INTO file_reservation_releases VALUES ({id}, NULL)"));
    }
    execute(&conn, &format!("INSERT INTO file_reservations VALUES
        (2000, 1, 0, {}), (2001, 1, 0, {}), (2002, 888, 0, {})", now + 1, now + 1, now + 2));
    execute(&conn, "INSERT INTO file_reservation_releases VALUES (2002, NULL)");
    execute(&conn, "COMMIT");
...
fn reservation_expiry_groups_can_span_many_pages_without_loss() {
    ...
    for id in 1..=total {
        ...
        let expiry = now + if id <= OVERVIEW_PAGE_ROWS * 2 { 1 } else { 2 + i64::try_from(id % 3).unwrap() };
        let legacy = if id % 7 == 0 { now } else { 0 };
        execute(&conn, &format!("INSERT INTO file_reservations VALUES ({id}, 1, 0, {expiry}, {legacy})"));
        if id % 5 == 0 {
            execute(&conn, &format!("INSERT INTO file_reservation_releases VALUES ({id}, NULL)"));
        }
...
fn cursors_include_negative_minimum_and_maximum_keys() {
    ...
    execute(&conn, &format!("INSERT INTO messages VALUES
        ({}, 1, 'high', 1, 0), ({}, 1, 'high', 1, 0)", i64::MIN, i64::MAX));
    execute(&conn, "BEGIN");
    for index in 0..OVERVIEW_PAGE_ROWS + 1 {
        let index = i64::try_from(index).expect("small fixture index");
        let rowid = if index == 0 { i64::MIN } else if index == i64::try_from(OVERVIEW_PAGE_ROWS).unwrap() {
            i64::MAX
        } else { -index };
        let message = if index % 2 == 0 { i64::MIN } else { i64::MAX };
        execute(&conn, &format!("INSERT INTO message_recipients
    ...
    assert_eq!((rows[0].unread, rows[0].urgent, rows[0].ack_overdue), (expected, expected, expected));
...
fn maximum_expiry_and_exact_page_boundary_terminate() {
    ...
    for id in 1..=OVERVIEW_PAGE_ROWS {
        let id = if id == OVERVIEW_PAGE_ROWS { i64::MAX } else { i64::try_from(id).unwrap() };
        execute(&conn, &format!("INSERT INTO file_reservations VALUES ({id}, 1, 0, {})", i64::MAX));
    }
    execute(&conn, "COMMIT");
...
fn empty_pending_set_never_fetches_message_metadata_or_release_history() {
    ...
    execute(&conn, "UPDATE message_recipients SET read_ts = 0, ack_ts = 0");
    execute(&conn, "INSERT INTO agents VALUES (99999, 777)");
    execute(&conn, "INSERT INTO messages VALUES (99999, 888, 'normal', 0, 0)");
    let (rows, work) = build_at(&conn, mcp_agent_mail_db::now_micros()).expect("completed inbox");
    assert_eq!(rows.len(), 5);
    assert!(rows.iter().all(|row| row.unread == 0 && row.urgent == 0 && row.ack_overdue == 0));
...
fn lookup_failure_after_multiple_pages_releases_snapshot_and_can_retry() {
    ...
    execute(&conn, &format!("UPDATE file_reservations SET expires_ts = {}", i64::MAX));
...
fn nonprogressing_cursor_and_oversized_lookup_are_rejected() {
    ...
    let result = scan_pages(&conn, &mut work, Scan {
        select: "SELECT project_id AS id FROM agents", predicate: "1 = 1",
        key: "project_id", params: &[], slot: 1,
