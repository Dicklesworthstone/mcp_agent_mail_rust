...
| A1 | done | `duplication_map.md`, `duplication_map.json` | scaffold/empty candidate state | implemented candidate inventory | artifact-only | JSON parsed with `python3 -m json.tool` |

## Gate Notes

- `cargo fmt --check`: passed after R1 and D1.
- `git diff --check`: passed after F1.
- `python3 -m json.tool refactor/artifacts/2026-04-24-proudridge-simplify/duplication_map.json`: passed after A1.
