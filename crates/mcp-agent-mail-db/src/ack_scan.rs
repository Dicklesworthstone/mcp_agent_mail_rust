...
const PAGE_SQL: &str = "\
WITH input AS (\
    SELECT ? AS resume, ? AS expected_generation, ? AS previous_upper_id, \
           ? AS previous_before_ts, ? AS current_before_ts, \
           ? AS previous_message, ? AS previous_agent\
), identity AS (\
...
impl AckScanPage {
    ...
    pub fn continuation_after(&self, consumed: usize) -> DbResult<Option<AckScanCursor>> {
        ...
        if let Some(row) = consumed.checked_sub(1).and_then(|index| self.rows.get(index)) {
...
fn parameters(
    ...
    Ok(vec![
        ...
        i64::try_from(page_size + 2)
            .map_err(|_| scan_error("page bound overflow"))?
            .into(),
    ])
}
...
pub async fn overdue_ack_page(
    ...
    let params = match parameters(&scope, cursor, overdue_before_ts, page_size) {
        Ok(params) => params,
    ...
    let result = conn.query(cx, PAGE_SQL, &params).await;
    crate::tracking::record_query(PAGE_SQL, crate::tracking::elapsed_us(started));
    match result {
...
mod tests {
    ...
    fn page(
        ...
        let params = parameters(scope, cursor, before, size).unwrap();
        let rows = conn.query_sync(PAGE_SQL, &params).unwrap();
    ...
    fn page_size_is_bounded_before_querying() {
        for size in [0, MAX_ACK_SCAN_PAGE_SIZE + 1, usize::MAX] {
            assert!(parameters("db", None, 10, size).is_err());
        }
        assert!(parameters("db", None, 10, MAX_ACK_SCAN_PAGE_SIZE).is_ok());
    }
    ...
    fn missing_generation_record_remains_readable_but_bad_generation_is_refused() {
        fixture(|conn| {
            ...
            .unwrap();
            let params = parameters("db", None, 10, 1).unwrap();
            assert!(
                decode_page("db".into(), conn.query_sync(PAGE_SQL, &params).unwrap(), 1).is_err()
            );
        });
    }
}
