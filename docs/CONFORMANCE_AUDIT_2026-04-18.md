...
Headline counts:
- Tools: 48 total = 42 supported-compatibility + 6 Rust-native. The sequential captured behavior fixture covers 37 compatibility tools; `fetch_topic`, `list_window_identities`, `sweep_stale_agents`, `summarize_recent`, and `fetch_summary` retain inventory/input-schema compatibility, Rust-owned non-empty descriptions, and focused coverage. The Rust implementation is authoritative over legacy fixtures.
...
| list_window_identities | yes | focused | python-parity | tests/conformance/fixtures/tool_descriptions.json | Python inventory/input-schema compatibility with a Rust-owned non-empty description; focused DB coverage proves active/expired identity filtering. |
| sweep_stale_agents | yes | focused | python-parity | tests/conformance/fixtures/tool_descriptions.json | Python inventory/input-schema compatibility with a Rust-owned non-empty description; focused DB coverage proves caller exclusion and active-reservation protection. |
...
| fetch_topic | yes | yes | python-parity | crates/mcp-agent-mail-conformance/tests/conformance.rs | Python inventory/input-schema compatibility with a Rust-owned non-empty description. Rust hardens Python's omitted-identity read: omission is accepted only when the current request carries FastMCP-verified transport authentication; otherwise an agent identity is required, and unread filtering always authenticates. Focused router coverage proves the identity boundary, case-insensitive lookup, body hydration, inbox filtering, and reply-topic inheritance. |
...
| summarize_recent | yes | focused | python-parity | tests/conformance/fixtures/tool_descriptions.json | Python inventory/input-schema compatibility with a Rust-owned non-empty description; focused summary response and persistence coverage. |
| fetch_summary | yes | focused | python-parity | tests/conformance/fixtures/tool_descriptions.json | Python inventory/input-schema compatibility with a Rust-owned non-empty description; focused cache and newest-first retrieval coverage. |
...
- Earlier same-day crate-doc count drift was folded into the documentation-alignment sweep; the live surface is now 48 tools / 25 resources, including the four restored compatibility tools `list_window_identities`, `sweep_stale_agents`, `summarize_recent`, and `fetch_summary`.
