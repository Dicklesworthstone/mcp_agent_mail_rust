...
fn reference(conn: &DbConn) -> Result<Vec<OverviewProject>, CliError> {
    ...
    let predicate = active_reservation_candidate_sql(
        has_file_reservations_released_ts_column(conn)?, "fr",
    );
    for row in conn.query_sync(&format!(
        "SELECT fr.id, fr.project_id FROM file_reservations fr
         WHERE ({predicate}) AND fr.expires_ts > ?"
    ), &[Value::BigInt(now)]).expect("reference reservations") {
    ...
    for row in conn.query_sync(sql, &[Value::BigInt(micros_ago(now, ACK_OVERDUE_THRESHOLD_US))])
        ...
        counts.ack_overdue = usize::try_from(integer(&row, "ack_overdue")?).expect("nonnegative overdue");
    }
...
fn cold_overview_recomputes_time_boundaries_without_a_write() {
    ...
    let threshold = micros_ago(now, ACK_OVERDUE_THRESHOLD_US);
    execute(&conn, "INSERT INTO projects VALUES (1, 'alpha')");
    execute(&conn, &format!("INSERT INTO messages VALUES (1, 1, 'high', 1, {threshold})"));
    execute(&conn, "INSERT INTO message_recipients VALUES (1, 1, NULL, NULL)");
    execute(&conn, &format!("INSERT INTO file_reservations VALUES (1, 1, 0, {})", now + 1));
...
fn cold_overview_releases_its_savepoint_after_a_query_failure() {
    ...
    execute(&conn, "CREATE TABLE message_recipients (message_id INTEGER, read_ts INTEGER, ack_ts INTEGER)");
    assert!(build(&conn).expect("read after failed collection").is_empty());
}
...
fn cold_overview_does_not_hide_mutations_below_a_max_timestamp() {
    ...
    let now = ACK_OVERDUE_THRESHOLD_US * 10;
    execute(&conn, "INSERT INTO projects VALUES (1, 'alpha')");
    execute(&conn, "INSERT INTO messages VALUES (1, 1, 'high', 1, 0), (2, 1, 'normal', 0, 0)");
    execute(&conn, "INSERT INTO message_recipients VALUES (1, 1, NULL, NULL), (2, 1, 999, 999)");
    ...
    assert_eq!((before[0].unread, before[0].urgent, before[0].ack_overdue), (1, 1, 1));
    ...
    execute(&conn, "INSERT INTO file_reservation_releases VALUES (1, NULL)");
...
fn live_overview_renderer_preserves_counts_and_full_envelopes() {
    let rows = vec![
        OverviewProject { slug: "alpha".into(), unread: 3, urgent: 2, ack_overdue: 1, reservations: 4 },
        OverviewProject { slug: "empty".into(), unread: 0, urgent: 0, ack_overdue: 0, reservations: 0 },
    ];
    let counts: serde_json::Value = serde_json::from_str(&render(&rows, true, OutputFormat::Json).expect("counts")).expect("counts JSON");
    let full: serde_json::Value = serde_json::from_str(&render(&rows, false, OutputFormat::Json).expect("full")).expect("full JSON");
    ...
    assert!(!render(&rows, true, OutputFormat::Toon).expect("TOON counts").is_empty());
    let empty: serde_json::Value = serde_json::from_str(&render(&[], true, OutputFormat::Json).expect("empty")).expect("empty JSON");
...
fn benchmark_linear_overview_against_current_main() {
    ...
    for iteration in 0..6 {
        ...
        for linear in [iteration % 2 == 0, iteration % 2 != 0] {
            ...
            let result = if linear { build(&conn) } else { reference(&conn) };
