...
fn archive_search_consistency_report_detects_archive_db_count_drift() {
    ...
    // (observed on 0.3.4 and still on 0.3.8) refuses a fresh open of this
    // reconstruct-produced file once an external DELETE has grown its
    // freelist ("unable to open database file"), while an already-open
    // runtime connection keeps serving — which is also the production shape
    // (a live pool predates at-rest mutations). The warm acquire below is
    // NOT redundant even though the connection is dropped immediately: it
    // drives the pool's one-time `initialize_sqlite_file_once` so the
    // runtime connection opens before the freelist grows. Removing it
    // (e9969ae0) made the later `search_messages_for_product` fail with
    // exactly that fresh-open refusal.
    block_on(|cx| {
