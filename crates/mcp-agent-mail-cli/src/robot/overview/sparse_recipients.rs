...
pub(super) fn try_collect(
    ...
    let Some(message_index) = full_index_with_prefix(conn, "messages", &["ack_required", "id"])? else {
        return Ok(false);
    };
    let Some(recipient_index) = full_index_with_prefix(conn, "message_recipients", &["ack_ts", "message_id"])? else {
        return Ok(false);
    };
    ...
    let sample = bounded_query(conn, &format!(
    ...
    scan_pages(conn, work, Scan {
        select: RECIPIENTS_SQL, predicate: UNREAD_FILTER,
        ...
        ids.sort_unstable();
        ids.dedup();
        let metadata = lookup_ids(conn, MESSAGES_SQL, "id", &[Value::BigInt(threshold)], &ids, work)?;
        work.message_lookup_rows += metadata.len();
        let mut messages = HashMap::with_capacity(metadata.len());
        for row in metadata {
            messages.insert(integer(&row, "id")?, (
                integer(&row, "project_id")?, integer(&row, "urgent")? != 0,
        ...
        for row in rows {
            ...
            let Some(&(project_id, urgent, overdue)) = row.get_by_name("message_id")
    ...
    let select = format!("{ACK_MESSAGES_SQL} INDEXED BY {}", quoted_identifier(&message_index));
    let mut ack_work = ScanWork::default();
    scan_pages(conn, &mut ack_work, Scan {
        select: &select, predicate: ACK_MESSAGES_FILTER, key: "id",
        params: &[Value::BigInt(threshold)], slot: 2,
    }, |rows, work| {
        ...
        for row in counts {
            ...
            let project_id = message_projects.remove(&id).ok_or_else(|| {
                CliError::Other("overview acknowledgement group was duplicate or unrequested".to_string())
            })?;
            let count = usize::try_from(integer(&row, "pending_count")?).map_err(|_| {
                CliError::Other("overview acknowledgement count was negative or overflowed".to_string())
            })?;
            let counts = project(projects, project_id);
...
mod tests {
    ...
    fn indexes(conn: &DbConn) {
        execute(conn, "CREATE INDEX ack_messages ON messages(ack_required, id)");
        execute(conn, "CREATE INDEX ack_recipients ON message_recipients(ack_ts, message_id)");
    }
    ...
    fn seed_history(conn: &DbConn, size: usize) {
        ...
        for start in (1..=size).step_by(OVERVIEW_PAGE_ROWS) {
            ...
            let messages: Vec<_> = (start..=end)
                .map(|id| format!("({id}, 1, 'normal', 0, 0)")).collect();
            let recipients: Vec<_> = (start..=end)
                .map(|id| format!("({id}, 1, 0, NULL)")).collect();
            execute(conn, &format!("INSERT INTO messages VALUES {}", messages.join(",")));
            execute(conn, &format!("INSERT INTO message_recipients VALUES {}", recipients.join(",")));
        }
        execute(conn, "COMMIT");
    }
    ...
    fn read_non_ack_history_does_not_materialize_recipients_or_message_lookups() {
        for history in [OVERVIEW_PAGE_ROWS * 4 + 1, OVERVIEW_PAGE_ROWS * 16 + 1] {
            ...
            execute(&conn, "INSERT INTO messages VALUES (99999, 77, 'high', 1, 0)");
            execute(&conn, "INSERT INTO message_recipients VALUES (99999, 1, NULL, NULL), (99999, 2, 0, NULL)");
            ...
            let orphan = rows.iter().find(|row| row.slug == "[unknown-project-77]").unwrap();
            assert_eq!((orphan.unread, orphan.urgent, orphan.ack_overdue), (1, 1, 2));
        }
    }
    ...
    fn read_acknowledgement_fanout_is_one_group_not_unbounded_recipient_rows() {
        ...
        for agent in 1..=fanout {
            execute(&conn, &format!("INSERT INTO message_recipients VALUES (1, {agent}, 0, NULL)"));
        }
        execute(&conn, "COMMIT");
    ...
    fn strict_deadlines_nulls_case_and_dangling_recipients_match_original() {
        ...
        execute(&conn, &format!("INSERT INTO messages VALUES
            ...
            (5, 1, 'URGENT', 0, 0), (6, 88, 'high', 0, 0)", threshold - 1, threshold + 1));
    ...
    fn acknowledgement_message_pages_include_signed_extremes_and_exact_boundaries() {
        for size in [OVERVIEW_PAGE_ROWS, OVERVIEW_PAGE_ROWS * 2 + 1] {
            ...
            for ordinal in 0..size {
                let id = if ordinal == 0 { i64::MIN } else if ordinal == size - 1 {
                    i64::MAX
                } else { -i64::try_from(ordinal).unwrap() };
                execute(&conn, &format!("INSERT INTO messages VALUES ({id}, 1, 'normal', 1, 0)"));
                execute(&conn, &format!("INSERT INTO message_recipients VALUES ({id}, 1, 0, NULL), ({id}, 2, 0, 0)"));
            }
            execute(&conn, "COMMIT");
            let (_, new) = compare_before_and_after_indexes(&conn);
    ...
    fn fully_acknowledged_history_skips_the_ack_message_scan() {
        ...
        assert!(build_at(&conn, NOW).unwrap().0.iter().all(|row| row.ack_overdue == 0));
    }
    ...
    fn unsupported_index_shapes_leave_counters_untouched_and_keep_the_fallback() {
        ...
        let (_dir, conn) = fixture();
        execute(&conn, "CREATE INDEX ack_messages ON messages(ack_required, id)");
        execute(&conn, "CREATE INDEX wrong_order ON message_recipients(message_id, ack_ts)");
        let mut projects = HashMap::new();
        let mut work = ScanWork::default();
        assert!(!try_collect(&conn, NOW, &mut projects, &mut work).unwrap());
        assert_eq!(work, ScanWork::default());
    ...
    fn quoted_indexes_work_inside_a_read_only_caller_transaction() {
        let (_dir, conn) = fixture();
        execute(&conn, "CREATE INDEX \"ack\"\"messages\" ON messages(ack_required, id)");
        execute(&conn, "CREATE INDEX \"ack\"\"recipients\" ON message_recipients(ack_ts, message_id)");
        execute(&conn, "INSERT INTO messages VALUES (1, 1, 'urgent', 1, 0)");
        execute(&conn, "INSERT INTO message_recipients VALUES (1, 1, 0, NULL)");
    ...
    fn indexed_counts_recompute_time_and_mutations_without_a_max_timestamp_shortcut() {
        ...
        indexes(&conn);
        let threshold = micros_ago(NOW, ACK_OVERDUE_THRESHOLD_US);
        execute(&conn, &format!("INSERT INTO messages VALUES (1, 1, 'high', 1, {threshold}), (2, 1, 'normal', 0, 0)"));
        execute(&conn, "INSERT INTO message_recipients VALUES (1, 1, NULL, NULL), (2, 1, 999, 999), (2, 2, 0, NULL)");
        ...
        execute(&conn, "UPDATE message_recipients SET read_ts = 1, ack_ts = 1 WHERE message_id = 1");
        let after = build_at(&conn, NOW + 1).unwrap().0;
        assert_eq!((after[0].unread, after[0].urgent, after[0].ack_overdue), (0, 0, 0));
    }
    ...
    fn benchmark_sparse_recipient_counts_against_pending_row_scan() {
        let (_dir, conn) = fixture();
        let history = std::env::var("AM_OVERVIEW_RECIPIENT_HISTORY")
            .map_or(24_000, |value| value.parse().expect("positive history size"));
        assert!(history > 0);
        seed_history(&conn, history);
        let active = history + 1;
        execute(&conn, &format!("INSERT INTO messages VALUES ({active}, 1, 'high', 1, 0)"));
        execute(&conn, &format!("INSERT INTO message_recipients VALUES ({active}, 1, NULL, NULL), ({active}, 2, 0, NULL)"));
        indexes(&conn);
        ...
        for iteration in 0..6 {
            for sparse in [iteration % 2 == 0, iteration % 2 != 0] {
                ...
                if sparse { new_times.push(elapsed); } else { old_times.push(elapsed); }
            }
        }
        old_times.sort_unstable();
        new_times.sort_unstable();
        eprintln!("native recipient phase only; read_non_ack_history={history}; old_median={:?}; new_median={:?}", old_times[3], new_times[3]);
    }
}
