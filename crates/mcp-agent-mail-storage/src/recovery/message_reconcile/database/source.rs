...
SELECT 1, mr.message_id, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, \
       ...
       CASE WHEN length(CAST(a.name AS BLOB)) <= ? THEN a.name ELSE NULL END, \
...
WHERE mr.message_id = ? \
ORDER BY row_kind, recipient_id LIMIT ?";
...
fn read_source(
    ...
    let limit = i64::try_from(MAX_RECIPIENTS)
        .map_err(|_| "archive recipient bound cannot be represented".to_string())?
        + 2;
    let rows = query(
        SOURCE_SQL,
        &[
...
mod tests {
    ...
    fn partial_cache_is_not_reinterpreted_as_the_legacy_default() {
        fixture(|cx, pool, _| {
            let conn = outcome(block_on(pool.acquire(cx))).unwrap();
            conn.execute_raw("UPDATE messages SET recipients_json = '{\"to\":[\"GreenStone\"]}' WHERE id = 901").unwrap();
            drop(conn);
            let error = prepare_message(cx, pool, 901).err().unwrap();
