...
mod tests {
    ...
    fn real_append_and_read_preserve_existing_bytes_and_names() {
        ...
        let mut file = journal.open_file("ack.jsonl", JournalFileMode::Read).unwrap();
    ...
    fn names_cannot_escape_or_select_alternate_streams_or_devices() {
        for name in [
            "", ".", "..", "../outside", "/outside", "a/b", "a\\b", "a:stream", "NUL",
            "con.txt", "COM1", "LPT9.txt", "trailing.", "trailing ", "a\0b",
        ] {
    ...
    fn hard_link_alias_is_rejected_without_changing_source_bytes() {
        ...
        for mode in [JournalFileMode::Read, JournalFileMode::Lock, JournalFileMode::Append] {
    ...
    fn symlink_leaf_and_directory_are_refused_without_touching_targets() {
        ...
        assert!(journal.open_file("new-entry", JournalFileMode::Append).is_err());
    ...
    fn retained_directory_never_redirects_an_open_to_its_replacement() {
        ...
        assert!(journal.open_file("ack.jsonl", JournalFileMode::Append).is_err());
        assert!(journal.sync().is_err());
        assert_eq!(std::fs::read_dir(root.path().join(DIRECTORY)).unwrap().count(), 0);
        assert_eq!(std::fs::read_dir(root.path().join("preserved")).unwrap().count(), 0);
    }
    ...
    fn newly_added_hard_link_invalidates_a_previously_open_file() {
        let root = tempfile::tempdir().unwrap();
        let journal = JournalDirectory::open(root.path(), DIRECTORY, true).unwrap();
        let file = journal.open_file("ack.jsonl", JournalFileMode::Append).unwrap();
        std::fs::hard_link(
