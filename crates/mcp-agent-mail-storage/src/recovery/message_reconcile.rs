...
pub fn reconcile_message_bundle(
    ...
    let created = entry.message.get("created").and_then(serde_json::Value::as_str)
    ...
    if entry.message.get("from").and_then(serde_json::Value::as_str) != Some(entry.sender)
        || entry.message.get("project_slug").and_then(serde_json::Value::as_str)
            != Some(archive.slug.as_str())
    {
        return Err(invalid("archive reconciliation identity does not match its project/sender"));
    }
    ...
    for kind in ["to", "cc", "bcc"] {
        let names = entry.message.get(kind).and_then(serde_json::Value::as_array)
            .ok_or_else(|| invalid(format!("archive reconciliation requires an explicit {kind} array")))?;
        for name in names {
            let name = name.as_str().ok_or_else(|| invalid("recipient must be a string"))?;
    ...
    if recipients != supplied || recipients.is_empty() {
        return Err(invalid("archive reconciliation recipient routing does not match frontmatter"));
    }
    if !entry.extra_paths.is_empty() {
        return Err(invalid("archive reconciliation does not authorize extra commit paths"));
    }
    ...
    let inbox = crate::render_message_bundle_content(
        &crate::redact_message_bcc_for_inbox(entry.message), entry.body_md,
    )?;
    let bytes = full.len().checked_mul(2)
        .and_then(|size| inbox.len().checked_mul(recipients.len()).and_then(|n| size.checked_add(n)))
    ...
    let (paths, _, _) = crate::message_paths_for_bundle(archive, entry.message, entry.sender, &recipients)?;
    let mut targets = vec![(paths.canonical.clone(), full.as_bytes()), (paths.outbox, full.as_bytes())];
    targets.extend(paths.inbox.into_iter().map(|path| (path, inbox.as_bytes())));
    let repo_root = crate::archive_repo_root_checked(archive)?;
    let rel_paths = targets.iter().map(|(path, _)| {
        crate::rel_path_cached(&archive.canonical_repo_root, path)
    ...
    let files_created = crate::with_project_lock(archive, || {
        ...
        let missing = targets.iter().map(|(path, expected)| {
            artifact_missing(path, expected)
        }).collect::<crate::Result<Vec<_>>>()?;
        if missing.iter().any(|missing| *missing) {
            crate::reject_canonical_message_id_collision(
                archive, entry.message, &paths.canonical, &full,
            )?;
        }
        let mut created = 0;
    ...
    let expected_oids = targets.iter().map(|(_, bytes)| {
    ...
    if git_commit_needed {
        ...
        crate::commit_paths_with_retry(
            &repo_root, config, &format!("repair: reconcile message {id} archive"), &refs,
        )?;
        ...
        if !head_contains(&verified, &rel_paths, &expected_oids)? {
            return Err(invalid("archive reconciliation Git verification failed; files retained"));
        }
    }
    Ok(ReconcileResult { files_created, git_commit_needed })
}

fn head_contains(repo: &Repository, paths: &[String], oids: &[Oid]) -> crate::Result<bool> {
    let head = match repo.head() {
        Ok(head) => head,
        Err(error) if matches!(error.code(), ErrorCode::UnbornBranch | ErrorCode::NotFound) => return Ok(false),
        Err(error) => return Err(error.into()),
    };
    let tree = head.peel_to_tree()?;
    for (path, expected) in paths.iter().zip(oids) {
        match tree.get_path(Path::new(path)) {
            Ok(entry) if entry.kind() == Some(ObjectType::Blob) && entry.id() == *expected => {}
...
fn artifact_missing(path: &Path, expected: &[u8]) -> crate::Result<bool> {
    if crate::path_existing_prefix_has_symlink(path)? {
        return Err(invalid("archive reconciliation refuses a symlinked artifact authority"));
    }
    match std::fs::symlink_metadata(path) {
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => return Ok(true),
        Err(error) => return Err(error.into()),
        Ok(metadata) if !metadata.is_file() => return Err(invalid("archive artifact is not a regular file")),
        Ok(metadata) if metadata.len() != expected.len() as u64 => {
            return Err(invalid("archive artifact conflicts with authoritative message; preserved"));
        }
        Ok(_) => {}
    }
    let file = mcp_agent_mail_core::disk::open_regular_file_no_follow(path)?;
    let mut observed = Vec::new();
    file.take(expected.len() as u64 + 1).read_to_end(&mut observed)?;
...
fn publish_missing_file(path: &Path, bytes: &[u8]) -> crate::Result<()> {
    ...
    if crate::path_existing_prefix_has_symlink(path)? {
        return Err(invalid("archive reconciliation authority changed before publication"));
    }
    let parent = path.parent().ok_or_else(|| invalid("archive artifact has no parent"))?;
    let mut staged = tempfile::Builder::new().prefix(".message-reconcile-").tempfile_in(parent)?;
    ...
    match staged.persist_noclobber(path) {
        ...
        Err(error) => {
            ...
            drop(kept.0);
            Err(std::io::Error::new(kind, format!(
                "archive publication refused; candidate retained at {}", kept.1.display(),
            )).into())
        }
    }
}
...
pub fn read_surviving_message(path: &Path) -> crate::Result<Option<(serde_json::Value, String)>> {
    ...
    match std::fs::symlink_metadata(path) {
        ...
        Err(error) => return Err(error.into()),
        Ok(metadata) if !metadata.is_file() || metadata.len() > MAX_MESSAGE_ARTIFACT_BYTES as u64 => {
            return Err(invalid("archive recovery source is nonregular or exceeds its byte bound"));
        }
        Ok(_) => {}
    }
    let file = mcp_agent_mail_core::disk::open_regular_file_no_follow(path)?;
    let mut bytes = Vec::new();
    file.take(MAX_MESSAGE_ARTIFACT_BYTES as u64 + 1).read_to_end(&mut bytes)?;
    if bytes.len() > MAX_MESSAGE_ARTIFACT_BYTES {
        return Err(invalid("archive recovery source grew past its byte bound"));
    }
    let text = std::str::from_utf8(&bytes).map_err(|_| invalid("archive recovery source is not UTF-8"))?;
...
mod tests {
    ...
    fn fixture() -> (tempfile::TempDir, Config, ProjectArchive, serde_json::Value, Vec<String>) {
        ...
        (dir, config, archive, message, vec!["GreenStone".into(), "RedFox".into()])
    }

    fn entry<'a>(message: &'a serde_json::Value, recipients: &'a [String]) -> MessageBundleBatchEntry<'a> {
        MessageBundleBatchEntry { message, body_md: "Keep Unicode: λ\n\n", sender: "BlueLake", recipients, extra_paths: &[] }
    }
    ...
    fn repair_materializes_and_commits_exact_copies_without_redelivery() {
        ...
        let paths = crate::message_paths_for_bundle(&archive, &message, "BlueLake", &recipients).unwrap().0;
        ...
        assert_eq!(read_surviving_message(&paths.outbox).unwrap().unwrap(), canonical);
    ...
    fn repair_rejects_any_conflicting_copy_before_creating_missing_files() {
        ...
        assert_eq!(std::fs::read(&paths.outbox).unwrap(), b"preserve conflicting evidence");
    }
    ...
    fn repair_existing_uncommitted_files_verifies_git_instead_of_skipping() {
        let (_dir, config, archive, message, recipients) = fixture();
        let paths = crate::message_paths_for_bundle(&archive, &message, "BlueLake", &recipients).unwrap().0;
        let full = crate::render_message_bundle_content(&message, entry(&message, &recipients).body_md).unwrap();
        crate::ensure_parent_dir(&paths.canonical).unwrap();
        std::fs::write(&paths.canonical, full.as_bytes()).unwrap();
        let repaired = reconcile_message_bundle(&archive, &config, entry(&message, &recipients)).unwrap();
    ...
    fn publication_collision_preserves_both_generations() {
        ...
        let candidates: Vec<PathBuf> = std::fs::read_dir(dir.path()).unwrap().map(|e| e.unwrap().path())
            .filter(|p| p.file_name().unwrap().to_string_lossy().starts_with(".message-reconcile-")).collect();
    ...
    fn repair_requires_original_identity_and_timestamp() {
        let (_dir, config, archive, message, recipients) = fixture();
        for (key, bad) in [("id", json!(0)), ("created", json!("bad")), ("from", json!("RedFox")),
            ("project_slug", json!("other")), ("bcc", json!(null))] {
            let mut invalid = message.clone();
            invalid[key] = bad;
            assert!(reconcile_message_bundle(&archive, &config, entry(&invalid, &recipients)).is_err(), "{key}");
        }
