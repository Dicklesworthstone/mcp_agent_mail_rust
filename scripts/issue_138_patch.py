...
root = Path.cwd()
lib = root / "crates/mcp-agent-mail-cli/src/lib.rs"
text = lib.read_text(encoding="utf-8")

old_classifier = """        // `Drain` is read-only and MUST run even while a live owner holds the
        // mailbox — reporting the supervised drain protocol is its whole purpose.
        DoctorCommand::Locks { .. } | DoctorCommand::Drain { .. } => true,
"""
new_classifier = """        // `Check`, `Locks`, and `Drain` are detector-only and MUST run while a
...
if text.count(old_classifier) != 1:
    raise SystemExit(f"classifier anchor count was {text.count(old_classifier)}, expected 1")
...
if text.count(unit_anchor) != 1:
    raise SystemExit(f"unit-test anchor count was {text.count(unit_anchor)}, expected 1")
text = text.replace(unit_anchor, unit_test + unit_anchor, 1)
...
e2e = root / "tests/e2e/test_doctor_check_live_server.sh"
e2e.write_text(script, encoding="utf-8")
e2e.chmod(0o755)
