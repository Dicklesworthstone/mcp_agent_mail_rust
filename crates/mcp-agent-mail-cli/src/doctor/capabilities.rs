...
pub fn build_report(tool_version: String, write_scopes: Vec<PathBuf>) -> CapabilitiesReport {
    ...
    for (k, v) in [
        ...
        ("HTTP_BEARER_TOKEN", "Active bearer token (read for token-rotation fixers)"),
...
fn build_detector_list() -> Vec<Detector> {
    vec![
        // Live operational
        det("server_port", "runtime_processes", "P1", "Port 8765 owner verification", 50, false, true),
        det("server_process_cpu", "runtime_processes", "P2", "Runaway CPU on listener", 100, false, false),
        det("server_http_health", "runtime_processes", "P1", "/health probe (offline-only when --online not passed)", 200, false, false),
        det("server_jsonrpc_health", "runtime_processes", "P1", "JSON-RPC health_check probe", 300, false, false),
        det("database", "db_state_files", "P0", "SQLite reachable", 30, false, true),
        det("db_file_sanity", "db_state_files", "P0", "PRAGMA quick_check + file size sane", 80, false, true),
        det("pool_init", "db_state_files", "P0", "Pool initializes without panic", 50, false, true),
        det("storage_root_writable", "archive_state_files", "P1", "Storage root writable", 20, false, true),
        det("archive_db_parity", "archive_state_files", "P1", "DB and Git archive in agreement", 200, false, false),
        det("foreign_key_integrity", "db_state_files", "P1", "PRAGMA foreign_key_check", 100, false, false),
        // Archive hygiene
        det("archive_hygiene", "archive_state_files", "P2", "Group-bucketed issue counts", 500, false, false),
        det("storage_root", "archive_state_files", "P1", "Storage root present + slug present", 30, false, true),
        det("storage_root_disk_space", "archive_state_files", "P2", "Free space above watermark", 20, false, true),
        det("storage_root_git_repo", "archive_state_files", "P0", "git/ exists at storage root", 30, false, true),
        det("storage_root_git_index_lock", "archive_state_files", "P1", "No stale .git/index.lock", 30, false, true),
        // Environment & config
        det("git_binary_path", "environment_toolchain", "P1", "git resolvable", 30, false, true),
        det("installed_agents", "environment_toolchain", "P3", "Detected agent CLIs", 50, false, false),
        det("binary_resolution", "environment_toolchain", "P1", "am binary on PATH and reachable", 30, false, true),
        det("legacy_python_alias", "environment_toolchain", "P2", "Old Python mcp_agent_mail shadowing", 30, false, true),
        det("stale_python_processes", "runtime_processes", "P0", "Stale Python servers", 100, false, false),
        det("path_order", "environment_toolchain", "P2", "~/.local/bin before /usr/local/bin", 20, false, true),
        det("binary_version", "environment_toolchain", "P3", "am --version matches latest known", 50, false, false),
        det("timestamp_format", "db_state_files", "P0", "DB timestamps in expected format (i64 vs TEXT)", 100, false, false),
        det("wal_mode", "db_state_files", "P1", "SQLite in WAL mode", 30, false, true),
        det("schema_version", "db_state_files", "P0", "Schema version current", 30, false, true),
        det("fts5", "db_state_files", "P2", "FTS5 module loadable", 30, false, true),
        det("guard_hooks", "guard_install", "P1", "Pre-commit hook installed", 50, false, true),
        det("mcp_config", "mcp_config_files", "P1", "Agent configs point at am", 100, false, false),
        det("mcp_config_token", "mcp_config_files", "P0", "Bearer token matches", 50, false, false),
        det("update_available", "environment_toolchain", "P3", "Newer release available", 1000, true, false),
        det("beads_issue_awareness", "environment_toolchain", "P3", ".beads/ integration", 30, false, true),
    ]
}
