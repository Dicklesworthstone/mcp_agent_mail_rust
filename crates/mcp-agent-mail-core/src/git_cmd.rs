...
impl<'a> GitCmd<'a> {
    ...
    fn run_once_inner(&self, budget: GitBudget) -> GitRunOutcome {
        let execute = || -> io::Result<GitRunOutcome> {
            ...
            let binary = resolve_git_binary().map_err(|error| {
                io::Error::other(format!("cannot resolve git binary: {error}"))
            })?;
...
mod tests {
    ...
    fn mutex_timeout_prevents_git_write_and_reacquisition_succeeds() {
        ...
        assert!(started.elapsed() < Duration::from_secs(5));
        assert!(!repo.join(".git/config").exists(), "timed-out work must not execute");
        drop(held);
        ...
        assert!(std::fs::read_to_string(repo.join(".git/config")).unwrap().contains("admitted"));
    }
    ...
    fn flock_timeout_releases_the_process_mutex_without_executing_git() {
        ...
        let mutex = GitRepoLocks::global().lock_for(&canonical);
        let guard = mutex.try_lock().expect("failed flock must release the mutex");
        drop(guard);
        drop(held);
        assert!(GitCmd::new(&repo).arg("--version").run().unwrap().status.success());
    }
    ...
    fn a_contended_repository_does_not_block_an_unrelated_git_invocation() {
        ...
        assert!(GitCmd::new(&second).arg("--version").run().unwrap().status.success());
    }
    ...
    fn retry_window_keeps_original_origin_and_does_not_cap_the_first_attempt() {
        ...
        assert_eq!(calls, 1, "an expired retry window must not launch attempt two");
        assert_eq!(error.kind(), io::ErrorKind::TimedOut);
